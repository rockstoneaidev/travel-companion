...
- Decisions log (2026-07-16, routing cost): **Google Maps is ~all of Google spend, and it's routing** — `routes_essentials` dwarfs `place_details`. Real BILLED spend is tiny (~$5 total; caching saves ~98%, so the scary "uncached" counterfactual is not money spent). Two free levers, both now in code: **(a) self-hosted OSRM behind the `Routing` port (E43)** — client is per-travel-mode (`OSRM_URL_FOOT`/`_BIKE`/`_DRIVE`; blank mode → Google), so the walking pilot can self-host alone; **NOT deployed** — the shared staging box (~500 MB RAM free, 2 vCPU, two other prod apps) would OOM-kill the neighbours building an OSRM graph, so it needs a small dedicated OSRM box (~€4–5/mo). **(b) OSM `opening_hours` first (shipped)** — the hours verify reads OSM's own tag before paying Google, answering the easy cases free and only calling Google for rich grammar or near-boundary times (timezone-safe). Flip OSRM on: `OSRM_URL_FOOT=…`, `ROUTING_DRIVER=osrm`, `config:clear`, parity spot-check (SERVER-DEPLOYMENT §"Self-hosted routing").
- Ops note (2026-07-16): the staging box fills with **old deploy images** — every deploy leaves a ~1.4 GB SHA-tagged image and they never auto-clean (was 136 images / 8 running, 91% disk). One-time `docker image prune -a -f` reclaimed 6 GB (→ 75%); images are all in `ghcr.io`, so rollback is a re-pull. Recurs without a retention cron (`docker image prune -a -f --filter until=168h`, weekly) — not yet added (persistent box config, ask first).
