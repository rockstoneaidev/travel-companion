...
| [#30](https://github.com/rockstoneaidev/travel-companion/issues/30) | Notification decision engine, budget & user tolerances | backend | **DONE 2026-07-14** — deterministic versioned policy (the LLM never picks the moment); 11 hard gates; 3/day hard cap; the urgent exception relaxes the cooldown and nothing else; `interruption_penalty` (SCORING §5.3) live; every denial written down, so §12.2's counterfactual is askable |
| [#31](https://github.com/rockstoneaidev/travel-companion/issues/31) | Push delivery: FCM/APNs, receipts & observability | platform | **DONE 2026-07-14** — `PushSender` port (FCM / log), `realtime` lane wakes up, deep-link to detail, receipts → the same feedback ledger (opened ⇒ `accepted`, swiped ⇒ `ignored`, **never** `dismissed`). **ROPA B13: no FCM DPA — sender unset by default.** |
