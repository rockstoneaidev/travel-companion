...
| **Interruption record** | `allowed`, `denied_by`, `notification_policy_version`, `priority`, `trace`, `sent_at`, `opened_at`, `dismissed_at` | `notifications` | No — but note it records **decisions we did NOT act on**, deliberately (§12.2's counterfactual). A row saying "we considered interrupting you about a bar and did not" is still a record about a person. |
| **Interruption tolerances** | `quiet_hours_start`, `quiet_hours_end`, `max_detour_minutes` | `users` | No — the user telling us how to treat them. Named, bounded, whitelisted (SCORING §9.2). |
...
| **Inferred day tempo** | `kind` (travel / sightseeing / relaxation), `net_displacement_m`, `span_m`, `distinct_cells` — a **derived summary of where somebody went on a given day** (E38) | `trip_segments` | No — but it is *personal data by inference*, and it must be said plainly: this is a classification of a person's days, computed from background location. It stores no coordinate (the numbers are distances, not places), which is deliberate — the summary is strictly less revealing than the pings it is drawn from. It dies with the trip: `cascadeOnDelete` on `trip_id`, so trip deletion (Art. 17) takes it. |
| **Inferred home zone** (proposal) | `h3_index` (res-8 cell, ~0.7 km²), `nights_observed`, `confidence`, `status` (E40) | `inferred_home_zones` | **Personal data by inference — an approximation of where someone lives** — and treated as the sensitive thing it is. It stores a *hexagon, never a coordinate*: the ROPA §8 "the coordinate is never written" promise is extended by this feature, not weakened, and the migration is written so a coordinate column does not exist to leak into. It only ever *proposes*; suppression activates on the user's confirmation, on the cell centroid (coarse by construction). `cascadeOnDelete` on `user_id`. |
...
| **Firebase Cloud Messaging (Google)** | **Processor** — push delivery | The **push token** and the **message body** (a place name and a time: *"the market near you closes in 22 minutes"*). A message body is content about where somebody is and what they are near. | 🇺🇸 US | **DPA REQUIRED AND NOT SIGNED — see B13.** Unset by default; the sender reaches nobody until `FCM_PROJECT_ID` is configured. |
