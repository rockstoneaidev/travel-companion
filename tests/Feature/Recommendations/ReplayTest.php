...
it('persists resolver_version and honest cost fields on the served trace', function () {
    ...
    expect($recommendations)->not->toBeEmpty()
        ...
        // No `llm_tokens`: the serve path cannot know them (COST.md §9, bug 3). The voice
        // is generated by a job dispatched and never awaited, so its tokens are spent in
        // another process minutes later — they now accrete to `cost_events` against this
        // recommendation's id. What stays on the trace is what the serve path DID know.
        ->and($recommendations[0]->cost)->toHaveKeys(['api_calls', 'rank_ms', 'scout_tiles_filled', 'scout_tiles_hit'])
        ->and($recommendations[0]->cost)->not->toHaveKey('llm_tokens')
