...
it('measures cost rather than asserting it', function () {
    ...
    // `llm_tokens` is GONE from the serve-path blob, and its absence is the assertion
    // (COST.md §9, bug 3). It was structurally always zero: the voice is generated by a
    // job dispatched and never awaited, so the tokens are spent later, in another
    // process, minutes after this row is written. The money lives in `cost_events` now,
    // where it can accrete to this recommendation's id from wherever it is actually spent.
    expect($recommendation->cost)->not->toHaveKey('llm_tokens');

    // Prove the meter actually counts, rather than being a dressed-up literal.
    // Drained first: the serve above legitimately left the weather call on it.
    $meter = app(CostMeter::class);
    $meter->drain();
    $meter->recordApiCall('places.googleapis.com', 'google_maps', 'place_details_essentials');
